// 滚轮刷新（根据滚动动作刷新元素位置）
// import { sysEvent } from '../../../core/script/common/sysEvent';
...
export default class reel_refresh extends cc.Component {
    ...
    clean_element_data(){
        this.element_data = {};
        this.end_element_idx = null;
        this.end_element_y = null;
    }
    ...
    getElementNode(result_idx){
        ...
        cc.error(result_idx, 'element_node', element_node, 'item_idx', this.item_idx, this.element_node_by_data)
        return null;
    }
    ...
    get_top_element_idx(){
        ...
        while (-(this.element_height/2) <= element_y && element_y <= (this.element_height/2)) {
            ...
            if(element_obj == null || element_obj.element_node == null){
                ...
                element_y = element_node.y - this.node.y;
            }
        }
        // 最上边看不到的元素 改成实际元素
        let top_element_idx = first_element_idx + this.element_num + 3;
    ...
    set_element_node(offset_y:any, index:number){
        ...
        if(element_obj == null){
            if(element_data){
                id = element_data.id;
                element_node = this.reel_element.create_element_select(element_data, this.panel_element_parent);
                if(element_data.result_idx != null){
                ...
                if(this.symbolWeight){
                    ...
                    let item = this.itemSelect.shift();
                    id = item.id;
                    element_node = this.reel_element.create_element_select(item, this.panel_element_parent);
                }else{
                    let template_element_num = this.reel_element.template_element.children.length;
                    // 设置一个随机1到N的变量。  除掉最后一个错误元素
                    id = Math.ceil(Math.random()*(template_element_num-1));
                    element_node = this.reel_element.create_element_select({id:id}, this.panel_element_parent);
                }
            }
            let obj = {
                id:id,
    ...
    setElementVisitable(flag:boolean){
        // for (const key in this.element_node_by_data) {
        //     if(this.element_node_by_data[key] != null){
        //         this.element_node_by_data[key].active = flag;
        //     }
        // }
        for (const key in this.element_obj) {
            if(this.element_obj[key] != null){
                this.element_obj[key].element_node.active = flag;
            }
        }
    }
    ...
    refresh_view(force?){
        ...
        if(this.last_set_y == y && force == null){
            return;
        }
        this.last_set_y = y;
        // 有几个不在视野了
